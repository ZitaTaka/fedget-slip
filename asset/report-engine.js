...
(function (global, document) {
  ...
  function inlineToMarkdown(node) {
    ...
    Array.from(node.childNodes).forEach(function (child) {
      ...
      if (t === 'ul' || t === 'ol') {
        // 表のセルなどインライン文脈中のリストは、崩れないようインラインHTMLとしてそのまま埋め込む
        out += child.outerHTML;
        return;
      }
  ...
  // caretのある「行」に相当するブロック要素(div/p/li/見出し/blockquote/表のセル)を探す。
  // 見つからない場合はroot自体を返す(=対象外として扱う)。
  function getBlockElement(node, root) {
    while (node && node !== root) {
      if (node.nodeType === 1 && /^(DIV|P|LI|H1|H2|H3|H4|H5|H6|BLOCKQUOTE|TD|TH)$/.test(node.tagName)) return node;
      node = node.parentNode;
    }
    return root;
  }
  ...
  function isCellElement(el) {
    return !!(el && el.nodeType === 1 && (el.tagName === 'TD' || el.tagName === 'TH'));
  }
  ...
  // blockを新しい要素(群)に置き換える。blockが表のセル(TD/TH)の場合はセル自体は
  // 残したまま中身だけを差し替える(セルをulなどで直接置き換えると表の構造が壊れるため)。
  // newElementsは配列: 先頭がblockの代わりになり、以降はその直後に続けて挿入される。
  function replaceBlockWith(block, newElements) {
    if (isCellElement(block)) {
      block.innerHTML = '';
      newElements.forEach(function (el) { block.appendChild(el); });
      return;
    }
    block.parentNode.replaceChild(newElements[0], block);
    var ref = newElements[0];
    for (var i = 1; i < newElements.length; i++) {
      ref.parentNode.insertBefore(newElements[i], ref.nextSibling);
      ref = newElements[i];
    }
  }
  ...
  function tryMarkdownEnterTrigger(e, root) {
    ...
    replaceBlockWith(block, [hr, nextBlock]);
  ...
  function init() {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      // Tab: リストのレベルUP・DOWNを優先(表のセル内に作られたリストも含む)、
      // リスト内でなければ表のセル間移動/行追加。
      if (e.key === 'Tab') {
